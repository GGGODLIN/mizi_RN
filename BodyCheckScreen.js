...
const BodyCheckScreen = props => {
  ...
  const handleSubmit = async () => {
    ...
    let url =
      'http://www.e9life.com/api/CheckResult/PostCheckDriverMapping';
          ...
          .then(res => {
            ...
            if (res.response.DriverCheck) {
              var nowDate = `${date.getFullYear()}-${date.getMonth() +
                1}-${date.getDate()}`;
              let url = `http://www.e9life.com/api/CheckResult/GetCheckDriverMappingSingle?DriverId=${
                obj_value.response.Id
              }&date=${nowDate}`;
...
有、已經看過醫生:
  ...
  const fetchDataModal = async () => {
    let url = `http://www.e9life.com/api/CheckItem/GetCheckDriver`;
