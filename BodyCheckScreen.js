...
const BodyCheckScreen = props => {
  ...
  const handleSubmit = async () => {
    ...
    let url =
      'http://ttsdapi.1966.org.tw/api/CheckResult/PostCheckDriverMapping';
  ...
  async function fetchData() {
    try {
      ...
      if (value !== null) {
        ...
        let url = `http://ttsdapi.1966.org.tw/api/DriverInfo/GetDriverCheck/${
          ...
          .then(res => {
            ...
            if (res.response.DriverCheck) {
              var nowDate = `${date.getFullYear()}-${date.getMonth() +
                1}-${date.getDate()}`;
              let url = `http://ttsdapi.1966.org.tw/api/CheckResult/GetCheckDriverMappingSingle?DriverId=${
                obj_value.response.Id
              }&date=${nowDate}`;
...
有、已經看過醫生:
  ...
  const fetchDataModal = async () => {
    let url = `http://ttsdapi.1966.org.tw/api/CheckItem/GetCheckDriver`;
