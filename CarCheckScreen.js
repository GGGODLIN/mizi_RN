          ...
          .then(res => {
            ...
            if (res.response.CarCheck) {
              var nowDate = `${date.getFullYear()}-${date.getMonth() +
                1}-${date.getDate()}`;
              let url = `http://ttsdapi.1966.org.tw/api/CheckResult/GetCheckCarMapping?CarId=${
...
不合格項目:
  ...
  const fetchDataModal = async () => {
    let url = `http://ttsdapi.1966.org.tw/api/CheckItem/GetCheckCarViewModel`;
  ...
  const handleSubmit = async () => {
    ...
    let url =
      'http://ttsdapi.1966.org.tw/api/CheckResult/PostCheckCarMapping';
